...
import sourabhkaushik.com.tech.credtask.customRecyclerViews.internal.CardStackSetting;
import sourabhkaushik.com.tech.credtask.customRecyclerViews.internal.CardStackSmoothScroller;
import sourabhkaushik.com.tech.credtask.customRecyclerViews.internal.CardStackState;
import sourabhkaushik.com.tech.credtask.customRecyclerViews.internal.CardStackState.Status;
import sourabhkaushik.com.tech.credtask.customRecyclerViews.internal.DisplayUtil;
import sourabhkaushik.com.tech.credtask.customRecyclerViews.StackFrom.*;

import static sourabhkaushik.com.tech.credtask.customRecyclerViews.internal.CardStackState.Status.Dragging;
import static sourabhkaushik.com.tech.credtask.customRecyclerViews.internal.CardStackState.Status.Idle;
...
public class CardStackLayoutManager
        ...
        if (state.isSwipeCompleted()) {
            ...
            new Handler().post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (topView != null) {
                        topView.setAlpha(1f);
                        ImageView imageView=topView.findViewById(R.id.imageBanner);
                        TextView title=topView.findViewById(R.id.title_tv);
    ...
    private void updateTranslation(View view, int index) {
        ...
        view.setAlpha(setting.maxAlpha);
        ImageView imageView=view.findViewById(R.id.imageBanner);
    ...
    public View getTopView() {
        View view=findViewByPosition(state.topPosition);
        if(view!=null){
            view.setAlpha(1f);
            ImageView imageView=view.findViewById(R.id.imageBanner);
            TextView title=view.findViewById(R.id.title_tv);
    ...
    public void setMaxAlpha(@FloatRange(from = 0.0f) float alpha) {
        if (alpha < 0.0f) {
            throw new IllegalArgumentException("ScaleInterval must be greater than or equal 0.0f.");
        }
        if (alpha >1f) {
            throw new IllegalArgumentException("ScaleInterval must be less than or equal 1f.");
        }
        setting.maxAlpha = alpha;
    }
